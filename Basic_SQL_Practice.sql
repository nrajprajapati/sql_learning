...
CREATE TABLE Director(Director_ID int,
...
From movie
group by genre
order by oldest_movie asc 
;

#4 Question
Select
genre as Genre,
sum(box_collection) as Total_Collection
From movie
group by genre ;
...
Sum(no_of_movies) as No_Of_Movie
FROM director
...
Select
lead_1,
count(Movie_name) as no_of_movie,
sum(box_collection) Total_Sales
From movie
group by lead_1 ;
